...
def resize_and_pad_3d(arr, target_size, order=1, foreground_mask=None):
    ...
    foreground_mask:
        Optional spatial boolean mask. For order > 0, foreground and background
        are interpolated separately and normalized by their support weights.
    Outputs
    -------
    arr_padded:
    ...
    if any(sf < 1.0 for sf in scale_spatial):
        channel_scale = (1.0, *scale_spatial)
        if order == 0 or foreground_mask is None:
            arr = zoom(arr, channel_scale, order=order)
        else:
            arr = interpolate_masked_regions(
                arr,
                foreground_mask,
                warp=lambda spatial: zoom(spatial, scale_spatial, order=order),
                nearest_warp=lambda spatial: zoom(
                    spatial, scale_spatial, order=0
                ),
            )
