...
def run_feature_calculator(anomaly_dir, synth_anomaly_dir, feature_calculator_func, config, cutout=True):
    ...
    for f in Path(synth_anomaly_dir).rglob("*.npy"):    # also checks all sub folders (for roi)
        anomaly_name = f.name
        metadata = getattr(config, "syn_anomaly_transformations", {}).get(anomaly_name, {})
        source_anomaly = metadata.get("source_anomaly", anomaly_name) if isinstance(metadata, dict) else anomaly_name
