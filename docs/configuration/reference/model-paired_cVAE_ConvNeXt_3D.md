...
Select with `config.model.set_model('paired_cVAE_ConvNeXt_3D')`. The table shows the actual defaults for this model variant. A dash in Default search means the parameter stays fixed unless you assign a compatible distribution through `config.model.search`.
