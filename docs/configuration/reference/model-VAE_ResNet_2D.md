...
Select with `config.model.set_model('VAE_ResNet_2D')`. The table shows the actual defaults for this model variant. A dash in Default search means the parameter stays fixed unless you assign a compatible distribution through `config.model.search`.
...
| `config.model.parameters.use_multires_skips` | `bool` | `False` | Use encoder features from multiple resolutions as skip connections. | — |
| `config.model.parameters.recon_weight` | `float` | `5.0` | Weight of the reconstruction loss. | — |
| `config.model.parameters.beta_kl_start` | `float` | `0.0` | Initial weight of the KL loss. | — |
| `config.model.parameters.beta_kl_max` | `float` | `0.1` | Maximum weight of the KL loss. | — |
