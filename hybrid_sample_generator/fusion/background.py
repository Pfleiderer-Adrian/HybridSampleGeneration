"""Background detection and preservation shared by fusion backends."""
...
def keep_control_background_after_fusion(
    ...
    if fused_image.shape != control_image.shape:
        raise ValueError(
            f"fused_image shape {fused_image.shape} does not match "
            f"control_image shape {control_image.shape}"
        )
    if fused_segmentation.shape != control_image.shape:
        raise ValueError(
            f"fused_segmentation shape {fused_segmentation.shape} does not match "
            f"control_image shape {control_image.shape}"
        )
    ...
    if background_mask.shape != control_image.shape[1:]:
        raise ValueError(
            f"background_mask shape {background_mask.shape} does not match "
            f"control spatial shape {control_image.shape[1:]}"
        )
    image = fused_image.copy()
    segmentation = fused_segmentation.copy()
    coordinates = (slice(None), *np.where(background_mask))
    image[coordinates] = control_image[coordinates]
...
def control_background_mask(
    ...
    """Return spatial pixels or voxels that belong to the control background."""
    control_image = np.asarray(control_image)
    if control_image.ndim < 2:
        raise ValueError(
            f"Expected channel-first image, got shape {control_image.shape}"
        )

    threshold = 0.0 if relative_bg_threshold is None else float(relative_bg_threshold)
    if threshold < 0.0:
        raise ValueError(
            f"relative_bg_threshold must be >= 0, got {relative_bg_threshold}."
        )
    ...
    for channel in range(control_image.shape[0]):
        ...
        high_values = (
            ...
            else np.asarray([low], dtype=np.float32)
        )
    ...
    per_channel = control_image <= np.asarray(cutoffs, dtype=np.float32).reshape(
        cutoff_shape
    )
...
def _robust_low_background_value(values: np.ndarray) -> float:
    ...
    min_count = int(
        np.count_nonzero(np.isclose(finite, min_value, rtol=0.0, atol=1e-6))
    )
    if min_count >= max(10, int(0.005 * finite.size)):
        return min_value
...
def _exterior_connected_mask(mask: np.ndarray) -> np.ndarray:
    ...
    if mask.ndim == 3:
        return np.stack(
            [_exterior_connected_mask(slice_mask) for slice_mask in mask],
            axis=0,
        )
    if not np.any(mask):
        return mask
...
__all__ = ["control_background_mask", "keep_control_background_after_fusion"]
