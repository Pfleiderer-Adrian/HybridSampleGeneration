 ...
 # (n,C,H,W)
    ...
    def generate_synth_sample_prior(
        ...
        *,
        ...
        Samples:
            z ~ N(0, I)  (scaled by s), then decode to image space.
        ...
        if out_hw is None:
            out_hw = getattr(self.cfg, "sample_hw", None) or getattr(self.cfg, "image_hw", None) or (256, 256)
        ...
        with torch.no_grad():
            ...
            for level in range(int(self.cfg.n_levels)):
                channels = 2 ** (level + 3)
                skip_hw = (H_pad // (2 ** level), W_pad // (2 ** level))
                zero_skips.append(torch.zeros((1, channels, *skip_hw), device=device, dtype=h_dec.dtype))
