...
from synthesizer.Matching import plan_hybrid_samples
from synthesizer.StudyRecords import OriginalSample, RealAnomaly, SyntheticAnomaly
...
class MatchingSelectionTests(unittest.TestCase):
    ...
    def test_local_stops_after_first_match_while_global_ranks_all_rois(self):
        self._populate()
        with patch("synthesizer.Matching._template_matching_prepared", side_effect=self._match) as match:
        ...
        with patch("synthesizer.Matching._template_matching_prepared", side_effect=AssertionError("cached pair")):
    ...
    def test_local_retries_invalid_and_overlapping_rois_and_caches_rejections(self):
        ...
        with patch("synthesizer.Matching._template_matching_prepared", side_effect=match_with_rejections) as match:
            plans = self._plan()
    ...
    def test_local_exhausts_invalid_pool_once_and_returns_no_plan(self):
        ...
        with patch("synthesizer.Matching._template_matching_prepared", return_value=(-2.0, None)) as match:
            plans = self._plan()
